...
void main(unsigned int argc, char *argv[])
{
	try {
		...
		CGrapheOperation ** ppqGRAGrapheConvert = (CGrapheOperation **) malloc (sizeof(CGrapheOperation *) * (argc - 1));
		if (ppqGRAGrapheConvert == nullptr)
			throw CException(ECHECALLOCATION, "Echec de l'allocation");
		...
		for (uiBoucleTableau = 0 ; uiBoucleTableau < argc -1 ; uiBoucleTableau++) {
			...
			printf("\n\n ------ GRAPHE NON ORIENTE ------ \n\n");
			ppqGRAGrapheConvert[uiBoucleTableau] = ppqGRAGraphe[uiBoucleTableau]->GRAConvertirGraphe();
			ppqGRAGrapheConvert[uiBoucleTableau]->GRAAfficherGraphe();
		...
		for (uiBoucleTableau = 0 ; uiBoucleTableau < argc -1 ; uiBoucleTableau++) {
			...
			delete(ppqGRAGrapheConvert[uiBoucleTableau]);
		}
		delete(ppqGRAGraphe);
		delete(ppqGRAGrapheInverse);
		delete(ppqGRAGrapheConvert);
